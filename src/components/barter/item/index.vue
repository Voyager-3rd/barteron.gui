	...
	<div :class="{
		...
		<template v-if="vType === 'tile' && !(hideInfo)">
			...
			<div class="row to" v-if="item?.tags?.length">
				<ul :style="exchangeList.length > 2 ? `--count: '+${ exchangeList.length - 2 }'; --len: ${ (exchangeList.length - 2).toString().length + 1 }` : ''">
		...
		<template v-if="vType === 'page'">
			...
			<div class="row block sep">
				<ExchangeList
					:key="`${item.address}_${exchangeList.map(l => l.id).join(',')}` /* to force update of the list after loading the offer */"
