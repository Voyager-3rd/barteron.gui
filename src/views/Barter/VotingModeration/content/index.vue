<template>
	...
	>
		...
		<p>{{ $t("votingModerationLabels.desired_score") }}</p>
		...
		<!-- <template v-if="!(statusesLoading || statusesLoadingError) && sdk.safeDealAvailable()">
			...
			<template v-if="!(waitingForPaymentConfirmation)">
				<div 
				...
				<template v-if="isBuyer && isStatus('1')">
					<div>
						...
						<div 
							v-if="status1BuyerCalculationList.length"
						...
						>
							...
							>
								<li 
									v-for="value in status1BuyerCalculationList"
									:key="value"
								>
									<span>{{ value }}</span>
								</li>
	...
	<v-content v-else>
		<section>
			<p>{{ $t('votingModerationLabels.user_has_no_access') }}</p>
		</section>
	</v-content>
</template>
